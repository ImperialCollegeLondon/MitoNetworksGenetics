...
double degradationRate(int w, double w_opt, double alpha, double mu){
	double degrate;

	degrate = alpha*(w - w_opt);
...
for (i=0; i< NREPEATS; i++){
	...
	while (t < TFinal){
		...
		// Degradation 
		// degradationRate(int w, double w_opt, double alpha, double mu)
		deg_rate = degradationRate(w_f + w_s, w_opt, alpha, mu);
