...
double degradationRate(int w, double w_opt, double mu){
	double degrate;
...
if(argc == 14){
	...
	printf("argc = %d\n", argc);
	printf("       %s  initial_seed param_block n_iters_per_block seed_inc w0_s m0_s w0_f m0_f xi beta gamma w_opt mu \n", argv[0]);
	return 0;
}
...
for (i=0; i< NREPEATS; i++){
	...
	while (t < TFinal){
		...
		// Degradation 
		// degradationRate(int w, double w_opt, double mu)
		deg_rate = degradationRate(w_f + w_s, w_opt, mu);
