...
double replicationRate(int w, int m, double delta, double mu, double w_opt, double alpha){
	double reprate;

	reprate = alpha*mu*(w_opt - w - delta*m)/((double)(w+m));
...
if(argc == 3){
	...
	m0_s = 110;
	w0_f = 187;
	m0_f = 80; // For h = 0.3, delta = 1.0, xi = 0.0
	...
	printf("       %s  initial_seed param_block n_iters_per_block seed_inc w0_s m0_s w0_f m0_f xi beta gamma alpha w_opt mu delta \n", argv[0]);
	return 0;
}
...
for (i=0; i< NREPEATS; i++){
	...
	while (t < TFinal){
		...
		// Replication
		rep_rate =replicationRate(w_f + w_s, m_f + m_s, delta, mu, w_opt, alpha);
