...
double replicationRate(int ws, int wf, int ms, int mf, double b, double kappa, double mu, double delta1, double delta2, double delta3, double delta4){
	double reprate;

	reprate = (mu+b*(kappa-(delta1*ws+delta2*wf+delta3*ms+delta4*mf)));
...
if(argc == 19){
	...
	printf("       %s  initial_seed param_block n_iters_per_block seed_inc w0_s m0_s w0_f m0_f xi beta gamma b kappa mu delta1 delta2 delta3 delta4 \n", argv[0]);
	return 0;
}
...
fprintf(fpStat,"xi = %f\n", xi);
fprintf(fpStat,"delta1 = %.10e\n", delta1);
fprintf(fpStat,"delta2 = %.10e\n", delta2);
fprintf(fpStat,"delta3 = %.10e\n", delta3);
fprintf(fpStat,"delta4 = %.10e\n", delta4);
...
for (i=0; i< NREPEATS; i++){
	...
	while (t < TFinal){
		...
		// Replication
		rep_rate =replicationRate(w_s, w_f, m_s, m_f, b, kappa, mu, delta1, delta2, delta3, delta4);
