...
double degradationRate(int w, int m, double kappa, double b, double delta, double mu){
	double degrate;

	degrate = mu + b*(w + delta*m - kappa);

	if (degrate < 0) {degrate = 0;}
	return degrate;
}
...
if(argc == 16){
	...
	printf("argc = %d\n", argc);
	printf("       %s  initial_seed param_block n_iters_per_block seed_inc w0_s m0_s w0_f m0_f xi beta gamma w_opt mu alpha \n", argv[0]);
	return 0;
}
...
for (i=0; i< NREPEATS; i++){
	...
	while (t < TFinal){
		...
		// Replication    
		//rep_rate =replicationRate(w_f + w_s, alpha);
		hazards[6] = w_f * mu; // wf --> 2wf
		hazards[7] = m_f * mu; // mf --> 2mf
		hazards[8] = w_s * mu; // ws --> 2wf
		hazards[9] = m_s * mu; // ms --> 2mf

		// Degradation 
		// degradationRate(int w, int m, double kappa, double b, double delta, double mu)
		deg_rate = degradationRate(w_f + w_s, m_f + m_s, kappa, b, delta, mu);
		hazards[10] = w_f * xi * deg_rate; // wf --> 0
		hazards[11] = m_f * xi * deg_rate; // mf --> 0
		hazards[12] = w_s * deg_rate; // ws --> 0
		hazards[13] = m_s * deg_rate; // ms --> 0
