...
for (i=0; i< NREPEATS; i++){
	...
	while (t < TFinal){
		...
		hazards[1] = m_s * (m_s - 1) * gamma/(2.0*Q); // ms + ms --> mf + mf
		hazards[2] = w_f * w_s * gamma; // wf + ws --> wf + wf
		hazards[3] = m_f * m_s * gamma/Q; // mf + ms --> mf + mf
		...
		// Physical cross-processes (fusion)
		hazards[14] = w_f * m_s * gamma/Q; // wf + ms --> wf + mf
		hazards[15] = m_f * w_s * gamma/Q; // mf + ws --> mf + wf
		hazards[16] = w_s * m_s * gamma/Q; // ws + ms --> wf + mf
