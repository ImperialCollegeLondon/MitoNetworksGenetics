...
#
# Stochastic simulations are run in parallel, generating many files. To read in each file and generate copy number and heteroplasmy summary statistics. 
# Mean and variance statistics will be computed online, so this code is scalable. It is rather slow due to the pandas operations.
...
if testing:
    max_iters = 2 # number of files expected
    data_dir = './test_data/p{:03d}'.format(param_index)
else:
    max_iters = 1000 # number of files expected
    data_dir = '/work/ja1109/networks/sing_birth_fus/param_sweep/100_param/p{:03d}'.format(param_index)

output_file_form = 'output_{0}_{1}.txt'

nrowskip = 10 # lines of metadata in output

nrep_per_job = 10 # number of repeats in each job


## Stream data and compute summary statistics

# thin a dataframe to every den rows
def thin_df(df, den):
    df.reset_index(inplace=True)
    return df.iloc[::den]


df_thinning = 1 # amount to thin each file by (if there is too much temporal resolution)
if df_thinning != 1:
    print('Thinning data temporally by factor {}'.format(df_thinning))


data1 = pd.read_csv(data_dir+'/'+output_file_form.format(param_index,0), skiprows=nrowskip, delimiter=',')
data1 = thin_df(data1, df_thinning)
...
def online_mean_var(x): 
    ...
    if x.counts < 2:
        ...
        x.var_n = x.M2_n/(x.counts - 1)
        if x.n > 0:
            x.var_h = x.M2_h/(x.counts - 1)
            x.var_fs = x.M2_fs/(x.counts - 1)            
        x.var_ns = x.M2_ns/(x.counts - 1)
        x.var_nf = x.M2_nf/(x.counts - 1)
        x.var_w = x.M2_w/(x.counts - 1)
        x.var_m = x.M2_m/(x.counts - 1)
    return x
...
for i in range(max_iters):
    ...
    if os.path.isfile(data_dir+'/'+output_file_form.format(param_index,i)): # check file exists        
        ...
        for j in range(nrep_per_job):
            ...
            rep_data = thin_df(rep_data, df_thinning) # thin down the data
            if len(rep_data)>0:                
