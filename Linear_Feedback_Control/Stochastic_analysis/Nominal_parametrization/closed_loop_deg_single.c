...
if(argc == 3){
	...
	m0_s = 140;
	w0_f = 373;
	m0_f = 160; // For h = 0.3, delta = 1.0, xi = 0.0
	...
	printf("       %s  initial_seed param_block n_iters_per_block seed_inc w0_s m0_s w0_f m0_f xi beta gamma kappa b mu delta \n", argv[0]);
	return 0;
}
//srand48(my_seed);
...
double TFinal = 750; // total amount of time to run simulation (days)

double DELTAT_STORE = 10.0; // Time lattice on which to store events
int NREPEATS = 100; // Number of repeats of the stochastic simulation
...
fprintf(fpStat,"xi = %f\n", xi);
fprintf(fpStat,"delta = %.10e\n", delta);
fprintf(fpStat,"beta = %.10e\n", beta);
fprintf(fpStat,"gamma = %.10e\n", gamma);
fprintf(fpStat,"kappa = %.10e\n", kappa);
fprintf(fpStat,"b = %.10e\n", b);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
// double hAddition;
// int meanwsInt, meanmsInt, meanwfInt, meanmfInt; 
// double meanws, meanms, meanwf, meanmf, meanh, meann; 
// double varws, varms, varwf, varmf, varh, varn; 
// double correct2, correct3, correct4, correct5, correct6, correct7, correct8;
//int Numberh = 0;
//int run;
//int nrun;
...
/*for(j = 0; j < nTimeSteps; j++) { //print things for these times
	for (run=0;run< NREPEATS;run++){
		ref = run*nTimeSteps + j;
		fprintf(fpStat, "%f,%d,%d,%d,%d,%d\n", j*DELTAT_STORE, run, wsStore[ref], msStore[ref],wfStore[ref],mfStore[ref]);
	}
}*/
...
// for(j = 0; j < nTimeSteps; j++) { //print things for these times
// 	//initialize means
// 	meanwsInt = meanmsInt = meanwfInt = meanmfInt = 0.0;
// 	meanws = meanms = meanwf = meanmf = meann = meanh = 0; Numberh = 0;
	
// 	correct2 = 0.0;

// 	for (run=0;run< NREPEATS;run++){
// 		ref = run*nTimeSteps + j;
// 		meanwsInt += wsStore[ref]; //do it like this to avoid adding floats which can lead to errors. So add the ints to an integer number
// 		meanmsInt += msStore[ref];
// 		meanwfInt += wfStore[ref]; 
// 		meanmfInt += mfStore[ref]; 


// 		if (wsStore[ref] + msStore[ref] + wfStore[ref] + mfStore[ref] != 0){ //otherwise h is not defined and we get a nan
// 			hAddition = (double)(msStore[ref] + mfStore[ref])/(double)(wsStore[ref] + msStore[ref] + wfStore[ref] + mfStore[ref]);
// 			meanh = KahanSum_fly(&meanh, hAddition, &correct2);	
// 			if(hAddition > 1){
// 				printf("hAddition = %f\n", hAddition);
// 				printf("%d %d %d %d\n", wsStore[ref], msStore[ref], wfStore[ref], mfStore[ref]);
// 				return 0;
// 			}
// 			Numberh += 1; //number of times we added to mean h.		
// 		} 
// 	}

// 	//mean values of w, m, h
// 	meanws = (double)meanwsInt/(double)NREPEATS;
// 	meanms = (double)meanmsInt/(double)NREPEATS;
// 	meanwf = (double)meanwfInt/(double)NREPEATS;
// 	meanmf = (double)meanmfInt/(double)NREPEATS;
// 	meann = meanws + meanms + meanwf + meanmf;
// 	meanh /= Numberh;

// 	if (meanh > 1.0){
// 	printf("%f %f %d\n", meanh, hAddition, Numberh);
// 	return 0;
// 	}

// 	//get variances
// 	varws = varms = varwf = varmf = varh = varn = 0.0;
// 	correct3 = correct4 = correct5 = correct6 = correct7 = correct8 = 0.0;
// 	nrun = 0;
// 	Numberh = 0; //again count the number of trajectories that have a well defined h.
// 	for(run = 0; run < NREPEATS; run++) {
// 		ref = run*nTimeSteps + j;
// 		varws = KahanSum_fly(&varws, (meanws-wsStore[ref])*(meanws-wsStore[ref]), &correct3);

// 		varms = KahanSum_fly(&varms, (meanms-msStore[ref])*(meanms-msStore[ref]), &correct5);
// 		varwf = KahanSum_fly(&varwf, (meanwf-wfStore[ref])*(meanwf-wfStore[ref]), &correct4);
// 		varmf = KahanSum_fly(&varmf, (meanmf-mfStore[ref])*(meanmf-mfStore[ref]), &correct6);
		
// 		nrun = wsStore[ref] + msStore[ref] + wfStore[ref] + mfStore[ref];  
// 		varn = KahanSum_fly(&varn, (meann- nrun)*(meann- nrun), &correct8);

// 		if (wsStore[ref] + msStore[ref] + wfStore[ref] + mfStore[ref] != 0){ //otherwise h not defined and get nan
// 			hAddition = pow(meanh - (double)(msStore[ref] + mfStore[ref])/(double)(wsStore[ref] + msStore[ref] + wfStore[ref] + mfStore[ref]),2);
// 			varh = KahanSum_fly(&varh,hAddition, &correct7);
// 			Numberh += 1;
// 		}
// 	}
...
// 	//print to file
// 	fprintf(fpStat, "%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%d\n", j*DELTAT_STORE, meanh, varh, 
// 																		meanws, meanms, meanwf, meanmf, 
// 																		varws, varms, varwf, varmf,
// 																		meann, varn, 
// 																		NREPEATS - Numberh);
// }
// fclose(fpStat); 
